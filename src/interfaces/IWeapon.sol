...
interface IWeapon {
   ...
   struct Metadata {
    ...
    function characterContract() external view returns (address _characterContract);
    ...
    function mintLegendaryWeapon(uint256 _attackPoints, uint256 _armorPoints, uint256 _sellPrice, uint256 _requiredExperience) external;
    ...
    function getSellInformation(uint256 _tokenId) external view returns(bool _onSale, uint256 _price, uint256 _requiredExperience);
    ...
    function addWeaponToCharacter(uint256 _weaponId, uint256 _characterId) external;
